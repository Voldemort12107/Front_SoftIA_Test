...
import { get_all_employer, lock_employer } from "../network";
...
const EmployeLock = () => {
      ...
      render: (id: number) => {
        return (
          <Button
            danger
            onClick={() => {
              lock_employer(id.toString(), connectedUser.access).then((res) => {
                if (res.code) {
                  console.log(res.code);
                  if (res.code === 404) {
                    navigate("*");
  ...
  return (
    <SideBar>
      <Title level={1}>Bloquer Employé</Title>
      <Table
        columns={columns}
        dataSource={data}
